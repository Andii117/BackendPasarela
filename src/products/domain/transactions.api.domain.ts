...
export class TransactionAPIDomain {
  constructor(
    ...
    async getTokentransaction() {
      const url = 'https://sandbox.wompi.co/v1/merchants/pub_test_vqyfdl6B3yg6HBohdCuaFhyIkAtOb6Ag';
    ...
    async RegisterCardtransaction(requestTransaction: RequestTransactionDTO) {
      ...
      const body = {
      number: requestTransaction.cardNumber, 
    ...
    async CompleteCardtransaction(requestTransaction: RequestTransactionDTO, tokentransaction: string, reference: string,acceptance_token: string) {
      ...
      signature: this.generateSignature(reference, requestTransaction.productPrice, currency, tokentransaction),
    ...
    generateSignature(reference: string, amount_in_cents: number, currency: string, tokentransaction: string ): string {
